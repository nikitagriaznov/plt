...
func TestDrawPo(t *testing.T) {
	...
	test := func(t1 testType) {
		...
		if fmt.Sprint(err) != fmt.Sprint(t1.err) {
			...
			_ = os.WriteFile(fmt.Sprintf("TestDrawPo%d.svg", testId), []byte(plot), 777)
		} else {
...
func TestDrawPoint(t *testing.T) {
	...
	doTest := func(id int, c testCase) {
		...
		plot, err := DrawPoint(c.TotalHeight, c.TotalWidth, c.xDivisionsQty, c.yDivisionsQty, c.X, c.Y, c.NameOfX, c.NameOfY)
		if fmt.Sprint(err) != fmt.Sprint(c.err) {
			t.Errorf("%sTest %d failed%s\twant: %s, got: %s\n", red, id, normal, c.err, err)
		} else if plot != c.plot {
			t.Errorf("%sTest %d failed%s\twant: %s, got: %s\n", red, id, normal, c.plot, plot)
			_ = os.WriteFile(fmt.Sprintf("TestDrawPoint%d.svg", id), []byte(plot), 777)
		} else {
			t.Logf("%sTest %d  success%s\t%s\n", green, id, normal, "")
		}
...
func TestDrawPointFromZero(t *testing.T) {
	...
	doTest := func(id int, c testCase) {
		t.Logf("Test %d\tstart", id)
		plot, err := DrawPointFromZero(c.TotalHeight, c.TotalWidth, c.xDivisionsQty, c.yDivisionsQty, c.X, c.Y, c.NameOfX, c.NameOfY)
		if fmt.Sprint(err) != fmt.Sprint(c.err) {
			t.Errorf("%sTest %d failed%s\twant: %s, got: %s\n", red, id, normal, c.err, err)
		} else if plot != c.plot {
			t.Errorf("%sTest %d failed%s\twant: %s, got: %s\n", red, id, normal, c.plot, plot)
			_ = os.WriteFile(fmt.Sprintf("TestDrawPointFromZero%d.svg", id), []byte(plot), 777)
		} else {
			t.Logf("%sTest %d  success%s\t%s\n", green, id, normal, "")
		}
